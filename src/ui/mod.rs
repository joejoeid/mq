...
pub fn draw(f: &mut Frame, state: &mut UiState) {
    ...
    for (i, c) in state.search_query.chars().enumerate() {
        if state.is_searching && i == state.search_cursor {
            query_spans.push(Span::styled(c.to_string(), Style::default().add_modifier(Modifier::REVERSED)));
        } else {
            query_spans.push(Span::raw(c.to_string()));
        }
    }
    if state.is_searching && state.search_cursor == state.search_query.chars().count() {
        query_spans.push(Span::styled(" ", Style::default().add_modifier(Modifier::REVERSED)));
    }
        
    let search_text = Paragraph::new(Line::from(query_spans))
        .block(search_block);
    ...
    let items: Vec<ListItem> = state.results.iter().map(|metadata| {
        let date_str = chrono::DateTime::from_timestamp(metadata.date, 0)
            .map(|dt| dt.format("%Y-%m-%d %H:%M").to_string())
            .unwrap_or_default();

        let content = Line::from(vec![
            Span::styled(format!("{:<15} ", &metadata.from.chars().take(15).collect::<String>()), Style::default().fg(Color::Cyan)),
    ...
    let preview_text = match &state.selected_preview {
        ...
        None => if state.results.is_empty() {
            "No results found."
        } else {
    ...
    if state.show_help {
        ...
        let help_text = vec![
            ...
            Line::from(""),
            Line::from(vec![Span::styled("NOTMUCH SEARCH SYNTAX", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))]),
            ...
            Line::from("  date:        : e.g. date:yesterday..today or date:2024-01-01..2024-02-01"),
            Line::from("  tag:         : e.g. tag:unread or tag:inbox"),
            Line::from("  folder:      : e.g. folder:Work/receipts"),
            Line::from("  body:        : e.g. body:\"tracking number\" (or just type the words directly)"),
            Line::from(""),
            Line::from(vec![Span::styled("LOGICAL OPERATORS & MODIFIERS", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))]),
            Line::from("  AND, OR, NOT : e.g. from:boss AND NOT tag:read"),
            Line::from("  * (wildcard) : e.g. subject:proj* (Note: wildcards work at the end of words)"),
        ];
